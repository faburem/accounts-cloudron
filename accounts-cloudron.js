 ...
 */
Accounts.oauth.registerService('cloudron');

/**
 * Client functionality (boilerplate).
 */
if (Meteor.isClient) {
  Meteor.loginWithCloudron = function(options, callback) {
    ...
    Cloudron.requestCredential(options, credentialRequestCompleteCallback);
  };
 ...
 */
  ...
  Accounts.addAutopublishFields({
    forLoggedInUser: _.map(
      ...
      function(subfield) {
        return 'services.cloudron.' + subfield;
      }),
    ...
    forOtherUsers: _.map(
       ...
       */
      _.without(Cloudron.whitelistedFields, 'email', 'verified_email'),
      function(subfield) {
        return 'services.cloudron.' + subfield;
      })
  });
